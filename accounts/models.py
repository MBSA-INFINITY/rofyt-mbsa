...
class CustomerManager(UserManager):
    def get_queryset(self, *args, **kwargs):
        ...
        return queryset
class CarOwnerManager(models.Manager):
    def get_queryset(self, *args, **kwargs):
        queryset = super().get_queryset(*args, **kwargs)
        queryset = queryset.filter(type=User.Types.CAROWNER)
        return queryset
...
class User(AbstractUser):
    ...
    aadhar_number = models.CharField(max_length=74, null=True, blank=True)
    aadhar_upload_front = models.TextField(null=True, blank=True)
    aadhar_upload_back = models.TextField(null=True, blank=True)

    pan_number = models.CharField(max_length=74, null=True, blank=True)
    pan_upload = models.TextField(null=True, blank=True)

    license_number = models.CharField(max_length=74, null=True, blank=True)
    license_upload_front = models.TextField(null=True, blank=True)
    license_upload_back = models.TextField(null=True, blank=True)
...
class CarOwner(User):
    objects = CarOwnerManager()

    class Meta:
        proxy = True

    def save(self, *args, **kwargs):
        self.type = User.Types.CAROWNER
        self.is_carowner = True
...
class CarOwnerPhoneVerify(models.Model):
    user = models.OneToOneField(CarOwner, on_delete=models.CASCADE)
...
@receiver(post_save, sender=CarOwner)
def print_only_after_deal_created(sender, instance, created, **kwargs):
    if created:
       CarOwnerPhoneVerify.objects.create(user=instance)
